...
export function SideNavigation({ children, ...props }: SideNavigationProps) {
  ...
  return (
    <motion.div
      className="fixed left-0 top-1/2 -translate-y-1/2 flex flex-col  rounded-r-lg gap-0.5 bg-foreground dark:bg-muted3 border"
      {...props}
      layout
      onMouseEnter={() => {
        setShow(true);
...
export function SideNavigationItem({ title, href, className, ...props }: SideNavigationItemProps) {
  return (
    <Link
      className={cn(
        " no-underline text-foreground  after:content-['']  after:absolute  after:left-0 after:bottom-0 after:h-[1.5px] after:w-full after:bg-foreground after:scale-x-0 after:origin-left after:rounded-lg after:transition-transform  after:duration-200 hover:after:scale-x-100",
        className,
      )}
      {...props}
      href={href}
