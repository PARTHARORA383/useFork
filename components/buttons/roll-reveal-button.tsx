...
const RollRevealContext = React.createContext<RollRevealContextProps | undefined>(undefined);
  ...
  return (
    <RollRevealContext.Provider value={{ hover, setHover }}>
      ...
      >
        <div
          className={cn(
            `absolute  inset-y-0 my-auto flex items-center justify-center h-8 w-8 rounded-full ${hover ? 'bg-foreground text-red-400' :' bg-red-400 text-white '}  `,
  ...
  return (
    <span
      className={cn(
        ...
        "group-hover:-translate-x-6 transition-transform duration-500 px-4 ml-4 bg-background  overflow-hidden h-10 flex items-center min-w-[120px] justify-center",
        className
      )}
    >
        <span
